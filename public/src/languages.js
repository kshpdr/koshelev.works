export const languages = {
  en: {
    text: "Hi! I am Denis and this is my website. <br /> I do here the most random stuff that my mind came up with just for fun.",
    stats: "Stats for",
    books: [
      { id: 1, title: "1948 - George Orwell", dateFinished: "26.01.2023" },
      { id: 2, title: "Brave New World - Aldous Huxley", dateFinished: "23.02.2023" },
      { id: 3, title: "Kafka on the Shore - Haruki Murakami", dateFinished: "21.03.2023" },
      { id: 4, title: "Unfaithfulness - Marina Trawkova", dateFinished: "26.03.2023" },
      { id: 5, title: "Great Expectations - Charles Dickens (Dropped)", dateFinished: "17.04.2023" },
    ]
  },
  de: {
    text: "Hey! Ich bin Denis und das ist meine Website. <br /> Ich mache hier die zufälligsten Dinge, die mir in den Sinn kommen, nur zum Spaß.",
    stats: "Statistik für",
    books: [
      { id: 1, title: "1948 - George Orwell", dateFinished: "26.01.2023" },
      { id: 2, title: "Schöne neue Welt - Aldous Huxley", dateFinished: "23.02.2023" },
      { id: 3, title: "Kafka am Strand - Haruki Murakami", dateFinished: "21.03.2023" },
      { id: 4, title: "Untreue - Marina Trawkova", dateFinished: "26.03.2023" },
      { id: 5, title: "Große Erwartungen - Charles Dickens (Dropped)", dateFinished: "17.04.2023" },
    ]
  },
  ru: {
    text: "Привет! Я Денис, и это мой сайт. <br /> Здесь я публикаю по фану рандомные вещи, которые мне пришли в голову.",
    stats: "Стата для",
    books: [
      { id: 1, title: "1948 - Джордж Оруэлл", dateFinished: "26.01.2023" },
      { id: 2, title: "О дивный новый мир - Олдос Хаксли", dateFinished: "23.02.2023" },
      { id: 3, title: "Кафка на пляже - Харуки Мураками", dateFinished: "21.03.2023" },
      { id: 4, title: "Неверность - Марина Травкова", dateFinished: "26.03.2023" },
      { id: 5, title: "Большие надежды - Чарльз Диккенс (Дропнул)", dateFinished: "17.04.2023" },
    ]
  },
};
